...
export class MtBaseComponent {
	...
	get isLoading(): boolean {
		// return this.comp.loading as boolean
		if (typeof this.comp['loading'] === 'function')
			return this.sm.getPropValue(this.comp, 'loading');
		return this.sm.getPropValue(this.comp, 'loading');
	 }
	...
	get hint() {
		return (this.comp.required && this.comp.type !== 'panel' && this.comp.type !== 'label') ? this.sm.translate(marker('comp_input.pflichtfeld')) : this.sm.getPropValue(this.comp, 'hint');
	}
	...
	onClick() {
		if (this.comp.onClick && !this.disabled) {
	...
	get tabIndex(): number {
		return this.comp.noTabIndex ? -1 : 1
	}
			...
			next: (comp) => {
				if (comp === this.comp && this.nameField) {
					if (this.nameField.nativeElement && this.nameField.nativeElement.focus) {
						...
						this.nameField.nativeElement.selectionStart = 0
						this.nameField.nativeElement.selectionEnd = 0
